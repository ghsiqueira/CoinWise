    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                ...
                @else
                    {{ __('Você ativou a autenticação de dois fatores com sucesso.') }}
                @endif
            @else
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Quando a autenticação de dois fatores estiver ativada, você será solicitado a inserir um token seguro e aleatório durante a autenticação. Você pode obter esse token no aplicativo Google Authenticator do seu telefone.') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        ...
                        @else
                            {{ __('A autenticação de dois fatores está agora habilitada. Escaneie o código QR a seguir usando o aplicativo autenticador do seu telefone ou insira a chave de configuração fornecida.') }}
                        @endif
                    </p>
                </div>
